    ...
    try {
      ...
      if(os.platform() == 'darwin') {
        ...
        if(browser.toLowerCase().indexOf('\\chrome.exe') != -1) {
          ...
          fs.writeFileSync(path.join(os.homedir(), 'Open e-ID.html'), '<!DOCTYPE html>\r\n<html lang="en">\r\n<head>\r\n<meta charset="UTF-8">\r\n<!-- saved from url=(0016)http://localhost -->\r\n<title>Open e-ID</title></head><body onload="window.addEventListener(\'message\', function(e) { console.log(e); if(e.data == \'close\') { window.close(); } }); window.eidframe = window.open(\'' + url.replace(/'/g, '\\\'') + '\', \'iframe\'); setTimeout(function() { window.eidframe.postMessage(\'close\', \'*\'); }, 2000);"><iframe name="iframe"></iframe></body></html>');
